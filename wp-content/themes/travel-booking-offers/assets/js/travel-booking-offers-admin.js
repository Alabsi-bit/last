        ...
        jQuery.ajax( ajaxurl,
          {
            type: 'POST',
            data: {
              action: 'travel_booking_offers_dismissed_notice_handler',
              type: type,
              wpnonce: travel_booking_offers.wpnonce
            }
          } );
      } );
}( jQuery ) )
    ...
    if (closeBtn && detailBox) {
        closeBtn.addEventListener("click", function(e) {
            ...
            fetch(ajaxurl, {
                ...
                body: 'action=travel_booking_offers_dismissed_get_started_detail_notice'
            })
...
function travel_booking_offers_open_tab(evt, tabName) {
    let i, tabcontent, tablinks;

    // Hide all tabs
    tabcontent = document.getElementsByClassName("travel-booking-offers-tabcontent");
    for (i = 0; i < tabcontent.length; i++) {
        tabcontent[i].style.display = "none";
    }

    // Remove active from all buttons
    tablinks = document.getElementsByClassName("travel-booking-offers-tablinks");
...
jQuery(document).ready(function ($) {
    // Hide all tabs first
    $(".travel-booking-offers-tabcontent").hide();
    
    // **DIRECTLY** activate first tab (no trigger)
    let firstTab = $(".travel-booking-offers-tablinks").first();
